...
async def login_for_access_token(
    form_data: dict = Body(...), db: Session = Depends(get_db)
):
...
async def read_users(
    ...
    _: User = Depends(utils.get_current_user),
):
...
async def read_patients(
    ...
    patients = utils.get_patients(
        db=db, first_name=first_name, last_name=last_name, skip=skip, limit=limit
    )
    return patients


@app.get("/pharmacies", response_model=List[schemas.Pharmacy])
async def read_pharmacies_by_name(
    name: str = None,
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db),
    ...
    pharmacies = utils.get_pharmacies(db=db, name=name, skip=skip, limit=limit)
    return pharmacies
